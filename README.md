...
- **Motion-tracker audio**: both sounds are synthesized in `audio_clips.h`
  ...
  audio is embedded; the tables are just measurements. Hear it without
  hardware: [docs/audio_demo.wav](docs/audio_demo.wav) is a rendered 8 m →
  0.3 m approach, regenerated by `tools/test_audio.py` — which also checks
  the pitch/tempo endpoints against the reference measurements (run it after
  touching the sound: `cd tools && python3 test_audio.py`).
